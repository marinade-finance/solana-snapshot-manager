...
import { DirectedStakeSdk, findVoteRecords, DirectedStakeVoteRecord } from '@marinade.finance/directed-stake-sdk'
...
export class VotesService {
  ...
  constructor(private readonly solanaServive: SolanaService, private readonly rdsService: RdsService) { }

  async getVoteRecordsFromChain(): Promise<DirectedStakeVoteRecord[]> {
    this.logger.log('Fetching mSOL votes from the chain...')
    const sdk = new DirectedStakeSdk({ connection: this.solanaServive.connection, wallet: Keypair.generate() })
